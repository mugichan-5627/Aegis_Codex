    ...
    "risks": [
        ...
        try:
            ...
            if parsed and "risks" in parsed and len(parsed["risks"]) >= 2:
                ...
                # Tag risks with Elastic citation
...
{{
    ...
    "reasoning": "2 sentences explaining why the arguments were weighted this way."
        ...
        transcript = [
            DebateMessage(role="bear", content=bear.get("argument", ""), round_number=1, confidence=float(bear.get("confidence", 0.75))),
            DebateMessage(role="bull", content=bull.get("argument", ""), round_number=1, confidence=float(bull.get("confidence", 0.55))),
...
class SwarmOrchestrator:
    """
    High-level orchestrator used by the Streamlit Executive Cockpit.
