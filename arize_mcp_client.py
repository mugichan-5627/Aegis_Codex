...
Supports live nested trace recording and visual rendering within the Streamlit UI console.
"""
...
# In-memory global store to capture active traces for Streamlit UI visualization
...
class ArizePhoenixTelemetry:
    ...
    def __init__(self, endpoint_url: Optional[str] = None):
        raw_endpoint = endpoint_url or os.getenv("PHOENIX_COLLECTOR_ENDPOINT") or os.getenv("PHOENIX_COLLECTOR_URL")
        if raw_endpoint:
    ...
    def initialize_collector(self) -> bool:
        ...
        if os.getenv("PHOENIX_COLLECTOR_ENDPOINT") or os.getenv("PHOENIX_COLLECTOR_URL") or os.getenv("ARIZE_API_KEY") or os.getenv("PHOENIX_API_KEY"):
    ...
    def _ship_telemetry(self, trace_payload: Dict[str, Any]) -> None:
        ...
        if os.environ.get("VERCEL") or os.environ.get("SERVERLESS") or os.environ.get("AEGIS_SYNC_TELEMETRY"):
            try:
                ...
                logger.error(f"Failed to ship telemetry synchronously in serverless environment: {str(e)}")
            return
